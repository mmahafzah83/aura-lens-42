  ...
  if (tap === "not_quite" || tap === "not_my_area") return (
    <div style={{ display: "grid", gap: 10 }}>
      ...
      <div style={{ display: "flex", gap: 8, flexWrap: "wrap" }}>
        {scopes[tap].map((value) => <button key={value} type="button" disabled={saving} onClick={() => void record(tap, value)} style={{ minHeight: 36, padding: "7px 10px", borderRadius: 4, border: "1px solid #E2E7EE", background: "#FFFFFF", color: "#0F1519", cursor: "pointer", fontFamily: "inherit" }}>{value === "just_this" ? S.just : S[value]}</button>)}
      </div>
    </div>
  );
