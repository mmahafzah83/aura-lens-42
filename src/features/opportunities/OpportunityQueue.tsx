...
export function OpportunityQueue() {
  ...
  const saveBar = async () => {
    ...
    if (!move || busy) return;
    setBusy(true); const { error } = await supabase.rpc("oe_move_save" as never, { p_move: move, p_place: editor === "place" ? (placePick.length ? placePick : null) : undefined } as never); setBusy(false);
  ...
  return <section className="oe-queue" dir={rtl ? "rtl" : "ltr"} lang={language} aria-busy={loading}>
    ...
    <nav className="oe-segments" aria-label={v("nav_aria")}>{(["today", "parked", "history"] as View[]).map((item) => <Button key={item} variant="ghost" aria-current={view === item ? "page" : undefined} onClick={() => setView(item)}><span>{v(`view_${item}`)}</span>{item === "today" && cards.length > 0 && <b style={mono}>{cards.length}</b>}{item === "parked" && data.parked.length > 0 && <b style={mono}>{data.parked.length}</b>}</Button>)}</nav>
