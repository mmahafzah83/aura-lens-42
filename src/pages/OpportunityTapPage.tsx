...
import { chairLabels, opportunityLabels } from "@/features/opportunities/labels";
  ...
  return <main dir={rtl ? "rtl" : "ltr"} style={{ minHeight: "100vh", background: "#F2F5F9", color: "#0F1519", display: "grid", placeItems: "center", padding: 16, fontFamily: rtl ? "Cairo, sans-serif" : "Inter, sans-serif" }}>
    <div style={{ width: "100%", maxWidth: 480 }}>
      <AuraCard hover="none" style={{ background: "#FFFFFF", border: "1px solid #E2E7EE", borderRadius: 20, padding: 20, boxShadow: "0 1px 2px rgba(15,21,25,.04)" }}>
        {card === undefined ? <p style={{ margin: 0, color: "#5B6673" }}>…</p> : !card ? <p style={{ margin: 0, color: "#5B6673" }}>{L.expired}</p> : <div style={{ display: "grid", gap: 16, lineHeight: rtl ? 1.9 : 1.55 }}>
          <span style={{ fontSize: 12, fontWeight: 700, color: "#5B6673" }}>{chairLabels[String(card.chair_type)]?.[language] ?? card.chair_type}</span>
          <h1 style={{ margin: 0, fontFamily: "inherit", fontSize: 23, lineHeight: rtl ? 1.7 : 1.3 }}>{card.title}</h1>
           {card.quote && <p style={{ margin: 0, color: "#5B6673", fontSize: 13 }}>“{card.quote}” {card.source_url && <a href={card.source_url} target="_blank" rel="noreferrer" style={{ color: "#0670C4" }}>{L.original}</a>}</p>}
          {outcomeDone ? <p style={{ margin: 0, color: "#12805C" }}>{L.noted}</p> : <TapRow token={token} language={language} source="email" initialTap={card.current_tap} autoTap={validAction} card={card} />}
          <Link to="/" style={{ color: "#5B6673", fontSize: 13, textDecoration: "underline" }}>{L.openAura}</Link>
        </div>}
      </AuraCard>
    </div>
  </main>;
