  ...
  const approveDraft = (idx: number) => {
    ...
    setTimeout(() => {
      ...
      if (drafts[idx]) {
        ...
        toast({ title: "Published — your presence just grew", description: "Copied to clipboard. Ready to publish." });
      }
      setSwipingIdx(null);
      ...
      <div className="grid grid-cols-1 lg:grid-cols-3 gap-8">
        ...
        <div className="lg:col-span-2 space-y-4">
          <div className="flex items-center justify-between">
            <div>
              ...
              <p className="text-xs text-muted-foreground/50 tracking-wide uppercase mt-0.5">Filtered by your sector & skills</p>
            </div>
            <button
              onClick={fetchNews}
                ...
                <div className="flex items-start gap-3">
                  <div className="flex-1 min-w-0">
                    <div className="flex items-center gap-2 mb-1.5 flex-wrap">
                      <span className={`text-xs tracking-widest uppercase px-2 py-0.5 rounded-md border ${typeColor(item.content_type)}`}>
                        {item.content_type}
                      </span>
                      <span className="text-xs text-primary/60">{item.relevance_tag}</span>
                    </div>
                      ...
                      <div className="mt-4 rounded-xl bg-muted/5 border border-border/15 p-4">
                        <div className="flex items-center justify-between mb-2">
                          <div className="flex items-center gap-2">
                            <span className={`text-xs px-2 py-0.5 rounded-md ${drafts[idx].audit_passed ? "bg-emerald-500/10 text-emerald-400 border border-emerald-500/20" : "bg-red-500/10 text-red-400 border border-red-500/20"}`}>
                              {drafts[idx].audit_passed ? "Audit ✓" : "Audit ✗"}
                            </span>
                            <span className="text-xs text-primary/50">{drafts[idx].brand_pillar_alignment}</span>
                            <span className="text-xs text-muted-foreground/40 capitalize">{drafts[idx].content_mix_category}</span>
                          </div>
        ...
        <div className="space-y-6">
          ...
          <div className="glass-card rounded-2xl p-6">
            ...
            <p className="text-xs text-muted-foreground/40 mb-4">70-20-10 content mix target</p>
                    ...
                    <div key={cat.label}>
                      <div className="flex items-center justify-between mb-1">
                        ...
                        <div className="flex items-center gap-2">
                          ...
                          <span className="text-xs text-muted-foreground/40">/ {cat.target}%</span>
                        </div>
                      </div>
