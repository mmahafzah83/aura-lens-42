// Aura MOVES — client twin of `supabase/functions/_shared/moves.ts`.
//
...
//
// This is the ONLY move table the client may read. `voiceOptions.ts` derives its
// library from here rather than maintaining its own list.
//
// RANGE IS NOT RANDOMNESS. Rotation is a constraint against repetition, always
// subordinate to a learned voice once one exists.

export type MoveBeat` onwards, and `scripts/check-vocabulary.mjs`
