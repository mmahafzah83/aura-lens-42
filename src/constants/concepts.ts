/**
 * THE DICTIONARY — CONCEPT REGISTRY.
 *
 * PASS V. Before this file, `vocabulary.ts` policed only HOW a number is
 * phrased. It could not police WHAT a word means or WHERE its number comes
 * from — which is how "captures" meant 174 on the Desk and 223 in the library
 * while the build stayed green.
 *
 * Every counted or named concept in Aura is ONE object here, carrying four
 * things:
 *
 *   the name     → `label` / `plural`, both languages
 *   the meaning  → `meaning`, one plain sentence a member would understand
 *   the number   → `countFn`, the ONE function allowed to produce it
 *   the phrasing → `formatter`, the ONE dictionary helper allowed to render it
 *
 * TWIN OF: `supabase/functions/_shared/concepts.ts`. The two files must stay
 * identical from the key list onwards; the vocabulary gate
 * compares them and fails the build on any drift. Edit one, edit the other.
 */
