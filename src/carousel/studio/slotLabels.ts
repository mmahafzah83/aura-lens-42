...
export function plainFailure(raw: string): string {
  ...
  if (raw.startsWith("INV-04")) return `${body.split(" — ")[0]} Each slide may carry one emphasis only.`;
  ...
  if (raw.startsWith("INV-02")) return body.replace("content overflows the canvas", "The text is too long for the slide");
  const leaks = /schema:|Array must contain|slots|INV-|slides\.\d+|\bZod\b/i;
