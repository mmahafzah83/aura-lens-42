...
// re-exported from `contentDNA.ts` so every generator reaches it in one place.
//
...
// from `src/`, so the table is mirrored and policed rather than duplicated by
// hand.
//
// It replaced three competing lists that never knew about each other:
//   - the 8 Flash post types in `generate-authority-content`
//   - the 7 acronym framework prompts in the same file
//   - the client's own free-text move library in `voiceOptions.ts`
// Their ids survive as aliases in `MOVE_ALIASES` so no caller had to change.
//
