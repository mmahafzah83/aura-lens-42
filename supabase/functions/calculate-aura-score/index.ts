...
serve(withObserve("calculate-aura-score", async (req) => {
  ...
  try {
    ...
    // Auth: accept user JWT (browser) OR service-role / cron (server-side callers
    // like detect-signals-v2). Mirrors voice-distill L51–95.
    ...
    if (isServiceRole || isCron) {
      ...
      const { data: { user: authUser }, error: authErr } = await userClient.auth.getUser(bearer);
      if (authErr || !authUser) {
        return new Response(JSON.stringify({ error: "Unauthorized" }), {
          status: 401, headers: { ...corsHeaders, "Content-Type": "application/json" },
        });
    ...
    EdgeRuntime.waitUntil(logError("calculate-aura-score", e, { user_id: null }));
