...
Deno.serve(withObserve("linkedin-identity-backfill", async (req) => {
  ...
  // The key is not trusted because it says it is a service key — it is proved
  // by using it against an endpoint only a real service key can open.
  const isService = await (async () => {
