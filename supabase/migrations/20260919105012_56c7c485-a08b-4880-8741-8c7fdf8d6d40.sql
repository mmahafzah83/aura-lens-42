...
BEGIN
  ...
  WITH latest AS (
    ...
    -- BUILD first: a stretch is a stretch even in his own sector. It must not
    -- be a seat he is barred from, but 'unknown' still counts — an
    -- unestablished requirement is not an exclusion.
    SELECT f.*,
      CASE
        WHEN COALESCE(f.eligibility_outcome,'unknown') <> 'excluded'
          ...
          THEN 'build'
        WHEN COALESCE(f.eligibility_outcome,'unknown') <> 'excluded' AND v_priority='known_for_one' AND f.chair_type IN ('speaking','media','learning') THEN 'build'
        WHEN COALESCE(f.eligibility_outcome,'unknown') <> 'excluded' AND v_priority='new_rooms' AND (f.chair_type IN ('room','advisory') OR f.discovery_kind='corporate_event_inference') THEN 'build'
        WHEN COALESCE(f.eligibility_outcome,'unknown') <> 'excluded' AND v_priority='out_of_sector' AND NOT f.core_sector THEN 'build'
