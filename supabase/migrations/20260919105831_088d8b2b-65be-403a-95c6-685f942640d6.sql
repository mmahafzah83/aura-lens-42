-- Generalise the ratified nationality rule.
UPDATE public.oe_notebook
SET value = 'nationality_mismatch',
    rule_text = 'Cannot take a seat that states a nationality or citizenship requirement he does not hold',
    rule_text_ar = 'لا يمكنه شغل مقعد يشترط جنسية لا يحملها'
WHERE entry_kind = 'rule' AND field = 'requirement' AND op = 'exclude' AND value = 'saudi_nationality';
...
DECLARE
  ...
  -- The bridge. A stretch only counts as 'build' when his own evidence can
  -- carry him into it: the function overlaps the work he has done, or the
  -- sector sits next door to the ones he has worked in.
  v_fn_re text := '(transform|digital|govern|operating model|operating-model|strategy|strategic|infrastructure|technolog|\mIT\M|data|\mAI\M|artificial intelligence|program management|portfolio|pmo|change management|modernis|moderniz|policy|regulat|board|committee|advisor)';
  v_sector_re text := '(energy|utilit|water|power|electric|renewab|government|public sector|public-sector|municipal|logistic|transport|supply chain|infrastructure|oil|gas|petro|mining|smart cit)';
BEGIN
  ...
  WITH latest AS (
    ...
    -- BUILD needs BOTH a step up AND a bridge from his demonstrated work.
    -- Seniority on its own is not a direction.
    SELECT f.*,
      CASE
