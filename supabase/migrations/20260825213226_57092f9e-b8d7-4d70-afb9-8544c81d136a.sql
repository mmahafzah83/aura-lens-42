...
WITH a(claim, ok, detail) AS (
    SELECT 'briefs_sent_today'::text,
        (EXISTS (
            SELECT 1 FROM daily_brief_snapshots
            WHERE daily_brief_snapshots.created_at > (now() - '26:00:00'::interval)
              AND daily_brief_snapshots.is_sent
        )),
        'founder-daily-brief must actually send, not merely run'::text
    UNION ALL
    SELECT 'no_silent_member'::text,
        NOT (EXISTS (
            SELECT 1 FROM cockpit_members
            WHERE cockpit_members.captures >= 3
              AND cockpit_members.posts_through_aura = 0
              AND cockpit_members.day_n >= 14
        )),
        'a member 14 days in with 3+ captures and 0 published is the core leak'::text
    UNION ALL
    SELECT 'faults_under_control'::text,
        ((SELECT count(*) AS count FROM ef_faults
    ...
    SELECT 'no_placeholder_reports'::text,
        NOT (EXISTS (
            SELECT 1 FROM diagnostic_profiles
    ...
    SELECT 'linkedin_addresses_are_confirmed'::text,
        NOT (EXISTS (
            SELECT 1 FROM diagnostic_profiles p
            JOIN linkedin_connections c ON c.user_id = p.user_id
            WHERE p.account_type = 'customer'::account_type
              AND COALESCE(c.source_status, ''::text) <> 'verified_by_read'::text
        )),
    ...
    SELECT 'every_customer_is_banded'::text,
        NOT (EXISTS (
            SELECT 1 FROM diagnostic_profiles
            WHERE diagnostic_profiles.account_type = 'customer'::account_type
              AND diagnostic_profiles.onboarding_completed
              AND diagnostic_profiles.seniority_band IS NULL
        )),
        'a member past onboarding must have a seniority band'::text
    UNION ALL
    SELECT 'question_banks_complete'::text,
        ((SELECT count(*) AS count FROM onboarding_questions
          WHERE onboarding_questions.sector IS NULL AND onboarding_questions.active)) = 27
        AND ((SELECT count(*) AS count FROM capability_dimensions
    ...
    SELECT 'report_written_from_evidence'::text,
        NOT (EXISTS (
            SELECT 1 FROM diagnostic_profiles p
            WHERE p.account_type = 'customer'::account_type
              AND p.brand_assessment_results IS NOT NULL
    ...
    SELECT 'morning_promise_is_kept'::text,
        (SELECT morning_promise_state.may_promise FROM morning_promise_state) AS may_promise,
    ...
    SELECT 'members_can_be_reached'::text,
        NOT (EXISTS (
            ...
            WHERE diagnostic_profiles.account_type = 'customer'::account_type
              AND diagnostic_profiles.onboarding_completed
              AND diagnostic_profiles.timezone IS NULL
        )),
    ...
    SELECT 'writes_land_where_intended'::text,
        NOT (EXISTS (
            ...
            WHERE e.sent_at > (now() - '30 days'::interval)
              AND e.metadata ? 'message_key'::text
              AND NOT (EXISTS (
                  SELECT 1 FROM lifecycle_email_log l
                  WHERE l.user_id = e.user_id
                    AND l.message_key = (e.metadata ->> 'message_key'::text)
              ))
        )),
    ...
    SELECT 'composer_language_parity'::text,
        ...
        END) AS count
        FROM operation_runs
        WHERE operation_runs.operation = 'studio_generate'::text
          AND operation_runs.started_at > (now() - '7 days'::interval))) = 2,
    ...
    SELECT 'composer_never_orders_impossible_number'::text,
        NOT (EXISTS (
            ...
            WHERE operation_runs.operation = 'studio_generate'::text
              AND operation_runs.started_at > (now() - '7 days'::interval)
              AND ((operation_runs.meta -> 'result'::text) ->> 'evidence_has_number'::text) = 'false'::text
              AND ((operation_runs.meta -> 'result'::text) ->> 'requested_ending'::text) = 'number'::text
        )),
    ...
    SELECT 'composer_guard_ran_last'::text,
        NOT (EXISTS (
            ...
            WHERE operation_runs.operation = 'studio_generate'::text
              AND operation_runs.started_at > (now() - '7 days'::interval)
              AND ((operation_runs.meta -> 'result'::text) ->> 'guarded_after_rotation'::text) = 'false'::text
        )),
