-- F3: lane_final and eligibility_fail are per-member facts. Move them to oe_matches.
ALTER TABLE public.oe_matches
  ADD COLUMN IF NOT EXISTS lane_final text,
  ADD COLUMN IF NOT EXISTS eligibility_fail text[];

UPDATE public.oe_matches m
SET lane_final = COALESCE(m.lane_final, o.lane_final),
    eligibility_fail = COALESCE(m.eligibility_fail, o.eligibility_fail)
FROM public.oe_opportunities o
WHERE o.id = m.opportunity_id;

CREATE INDEX IF NOT EXISTS oe_matches_user_lane_idx ON public.oe_matches(user_id, lane_final);

-- Readers rebuilt against the per-member column.
...
-- The shared table no longer carries per-member verdicts.
ALTER TABLE public.oe_opportunities DROP COLUMN IF EXISTS lane_final;
ALTER TABLE public.oe_opportunities DROP COLUMN IF EXISTS eligibility_fail;