...
BEGIN
  ...
  interleaved AS (SELECT r.*,((r.purpose_rank-0.5)/GREATEST(COALESCE((v_weights->>r.purpose)::numeric,0.01),0.01))/GREATEST(r.goal_weight*r.place_weight,0.1) interleave_rank FROM ranked r), selected AS (SELECT i.* FROM interleaved i WHERE i.lane_final='act' OR i.lane_rank<=3)
...
revoke execute on function public.oe_place_weight(uuid, text, text) from public;

update public.oe_surfaces s
set cadence = 'monthly'
from public.oe_entities e
where e.id = s.entity_id
  and coalesce(s.cadence,'') <> 'monthly'
  and nullif(trim(e.country),'') is not null
  and upper(e.country) not in ('SA','AE','QA','KW','BH','OM');