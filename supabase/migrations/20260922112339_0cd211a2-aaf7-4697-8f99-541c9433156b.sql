...
REVOKE EXECUTE ON FUNCTION public.oe_card_kinds() FROM PUBLIC, anon, authenticated;
...
BEGIN
  ...
  valid AS (
    SELECT o.*,m.lane_final,m.scores,m.requirement_check,m.met_count,m.total_count,COALESCE(m.purpose,'explore') purpose,
      ...
      -- Proof: how much of what this one asks for his own record already answers.
      ...
      -- Interest: what he reads and wants. Ranking only, never proof, never a gate.
    ...
    WHERE o.alive AND o.kind = ANY(v_kinds) AND m.lane_final IN ('act','write') AND o.quote_verified AND NULLIF(trim(o.evidence_quote),'') IS NOT NULL AND NOT ('place'=ANY(COALESCE(m.eligibility_fail,'{}'::text[])))
