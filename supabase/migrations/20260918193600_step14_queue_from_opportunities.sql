  ...
  WITH latest_matches AS (
    ...
    SELECT o.*, m.scores, m.requirement_check, m.met_count, m.total_count,
      COALESCE((
        ...
        WHERE f.user_id=v_uid AND f.id IN (
          ...
          WHERE cite->>'kind'='face' AND cite->>'id' ~* '^[0-9a-f-]{36}$'
    ...
    WHERE o.alive AND o.lane_final IN ('act','write')
      ...
      AND EXISTS (
        ...
        JOIN oe_faces f ON f.id=(cite->>'id')::uuid AND f.user_id=v_uid
        WHERE cite->>'kind'='face' AND cite->>'id' ~* '^[0-9a-f-]{36}$' AND NULLIF(trim(f.summary),'') IS NOT NULL
      )
      AND NOT EXISTS (
        SELECT 1 FROM oe_serves s
        WHERE s.user_id=v_uid AND s.opportunity_id=o.id
...
CREATE OR REPLACE FUNCTION public.oe_app_render(p_opportunity uuid)
RETURNS jsonb
LANGUAGE plpgsql
SECURITY DEFINER
SET search_path=public
...
BEGIN
  IF v_uid IS NULL THEN RAISE EXCEPTION 'authentication required'; END IF;
  SELECT * INTO v_opp FROM oe_opportunities WHERE id=p_opportunity AND alive AND lane_final IN ('act','write');
  IF NOT FOUND THEN RAISE EXCEPTION 'opportunity unavailable'; END IF;
  SELECT * INTO v_match FROM oe_matches WHERE opportunity_id=p_opportunity AND user_id=v_uid ORDER BY judged_at DESC LIMIT 1;
  ...
  FROM oe_faces f WHERE f.user_id=v_uid AND f.id IN (
    SELECT (cite->>'id')::uuid FROM jsonb_array_elements(COALESCE(v_match.scores->'cites','[]'::jsonb)) cite
    WHERE cite->>'kind'='face' AND cite->>'id' ~* '^[0-9a-f-]{36}$'
  ) AND NULLIF(trim(f.summary),'') IS NOT NULL;
  ...
  IF NOT v_opp.quote_verified OR NULLIF(trim(v_opp.evidence_quote),'') IS NULL OR jsonb_array_length(v_evidence)=0 THEN
    INSERT INTO ef_error_log(function_name,user_id,severity,error_message,context)
    VALUES('oe_app_render',v_uid,'error','Opportunity render refused because its reason was not grounded',jsonb_build_object('opportunity_id',p_opportunity,'verified_quote',v_opp.quote_verified,'evidence_count',jsonb_array_length(v_evidence)));
  ...
  VALUES(v_uid,NULL,p_opportunity,'app',v_opp.lane_final,v_why) RETURNING id INTO v_serve;
...
CREATE OR REPLACE FUNCTION public.oe_app_decide(p_opportunity uuid,p_action text,p_scope text DEFAULT NULL,p_scope_value text DEFAULT NULL,p_truth text DEFAULT NULL)
RETURNS jsonb
LANGUAGE plpgsql
SECURITY DEFINER
SET search_path=public
...
BEGIN
  ...
  SELECT * INTO v_serve FROM oe_serves WHERE opportunity_id=p_opportunity AND user_id=v_uid AND channel='app' AND tap IS NULL ORDER BY shown_at DESC LIMIT 1;
  IF NOT FOUND THEN RAISE EXCEPTION 'serve unavailable'; END IF;
  SELECT * INTO v_opp FROM oe_opportunities WHERE id=p_opportunity;
  ...
  IF p_truth IS NOT NULL THEN
    ...
    IF p_truth='dead_route' THEN UPDATE oe_opportunities SET route_dead=true WHERE id=p_opportunity;
    ELSIF p_truth='quote_absent' THEN UPDATE oe_opportunities SET quote_verified=false WHERE id=p_opportunity;
    ELSE UPDATE oe_opportunities SET alive=false WHERE id=p_opportunity; END IF;
    ...
    VALUES(CASE WHEN p_truth='listing_page' THEN 'aggregator_fingerprint' WHEN p_truth='already_happened' THEN 'recurring_event' ELSE 'route_pattern' END,
      jsonb_build_object('code',p_truth,'opportunity_id',p_opportunity,'feed_id',v_opp.feed_id),COALESCE(v_opp.route_url,v_opp.source_url),0.8);
