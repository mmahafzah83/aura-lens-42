ALTER TABLE public.oe_direction
  ADD COLUMN IF NOT EXISTS goal_secondary text[] NULL;

ALTER TABLE public.oe_direction DROP CONSTRAINT IF EXISTS oe_direction_goal_secondary_check;
ALTER TABLE public.oe_direction ADD CONSTRAINT oe_direction_goal_secondary_check
  CHECK (
    goal_secondary IS NULL
    OR goal_secondary <@ ARRAY['income_from_expertise','advancement','visibility','relationships','knowledge']::text[]
  );

DROP FUNCTION IF EXISTS public.oe_goal_save(text, boolean);
CREATE FUNCTION public.oe_goal_save(
...
BEGIN
  ...
  END IF;
  IF p_secondary IS NOT NULL AND EXISTS (
    SELECT 1 FROM unnest(p_secondary) value
    WHERE value NOT IN ('income_from_expertise','advancement','visibility','relationships','knowledge')
  ) THEN
    RAISE EXCEPTION 'unknown secondary goal';
  END IF;
  ...
  INSERT INTO oe_direction(user_id, goal, goal_secondary, goal_confirmed_at, goal_expires_at, window_declared_on, window_expected_by)
  VALUES (
    v_uid,
    p_goal,
    CASE WHEN p_goal IS NOT NULL THEN p_secondary END,
  ...
  ON CONFLICT (user_id) DO UPDATE SET
    ...
    goal_secondary = CASE WHEN EXCLUDED.goal IS NOT NULL THEN p_secondary ELSE oe_direction.goal_secondary END,
...
REVOKE ALL ON FUNCTION public.oe_goal_save(text, boolean, text[]) FROM public, anon;
GRANT EXECUTE ON FUNCTION public.oe_goal_save(text, boolean, text[]) TO authenticated;
...
BEGIN
  ...
  SELECT jsonb_build_object(
    ...
    'history',COALESCE((SELECT jsonb_agg(jsonb_build_object(
      'shown_at',x.shown_at,'lane',x.lane,'tap',x.tap,'tap_scope',x.tap_scope,'tap_scope_value',x.tap_scope_value,
      'tapped_at',x.tapped_at,'truth_code',x.truth_code,'outcome',x.outcome,'outcome_at',x.outcome_at,
      'title',o.title,'issuer_name',i.canonical_name,'location',o.location,'presentation_line',lm.presentation_line
    ) ORDER BY x.shown_at DESC)
      FROM (SELECT * FROM oe_serves WHERE user_id=v_uid ORDER BY shown_at DESC LIMIT 60) x
      LEFT JOIN oe_opportunities o ON o.id=x.opportunity_id
      LEFT JOIN oe_issuers i ON i.id=o.issuer_id
      LEFT JOIN LATERAL (SELECT m.presentation_line FROM oe_matches m WHERE m.user_id=v_uid AND m.opportunity_id=o.id ORDER BY m.judged_at DESC LIMIT 1) lm ON true
    ),'[]'::jsonb),
