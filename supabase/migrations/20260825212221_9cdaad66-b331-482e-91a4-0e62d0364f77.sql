...
WITH a(claim, ok, detail) AS (
        ...
        UNION ALL
         -- Same boundary: drafts generated before the contract could not have been guarded by it.
         SELECT 'composer_drafts_are_complete'::text, NOT EXISTS (
           SELECT 1 FROM operation_runs WHERE operation='studio_generate'
             AND started_at > greatest(now() - interval '7 days', timestamptz '2026-08-25 18:56:00+00')
        ...
        UNION ALL
         -- Law #85 shipped 2026-08-25 18:56 UTC. Rows before it were written by the
         -- deleted TRIGGER heuristic and describe a contract that did not yet exist.
         SELECT 'composer_contract_held'::text, NOT EXISTS (
           SELECT 1 FROM output_leak_log
           WHERE created_at > greatest(now() - interval '7 days', timestamptz '2026-08-25 18:56:00+00')
        ...
        UNION ALL
         SELECT 'composer_never_orders_impossible_number'::text, NOT EXISTS (
           SELECT 1 FROM operation_runs WHERE operation='studio_generate'
             AND started_at > now() - interval '7 days'
             AND meta->'result'->>'evidence_has_number' = 'false'
             AND meta->'result'->>'requested_ending' = 'number'
         ), 'Law #86 — a close that demands a figure, drawn against digit-free evidence, cannot be satisfied'::text
        UNION ALL
         SELECT 'composer_guard_ran_last'::text, NOT EXISTS (
           SELECT 1 FROM operation_runs WHERE operation='studio_generate'
             AND started_at > now() - interval '7 days'
             AND meta->'result'->>'guarded_after_rotation' = 'false'
