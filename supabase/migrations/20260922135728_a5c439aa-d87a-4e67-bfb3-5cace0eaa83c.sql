-- "wrong sector" was offered on screen but thrown away on the way in.
...
BEGIN
  ...
  VALUES(v_uid,v_serve.card_id,p_card,v_tap,CASE WHEN p_scope IN ('issuer','level','place','sector','type','just_this') THEN p_scope END,p_scope_value,'inapp',now()) ON CONFLICT DO NOTHING;
...
-- The member's own place, so the question can offer it by name and no city is
-- ever written into the product's words.
...
AS $function$
  ...
  city as (
    ...
    where s.user_id = p_user and nullif(trim(s.location),'') is not null
    order by s.created_at desc limit 1
  ),
  country as (
    select c.iso2, c.name_en, c.region_codes from oe_ref_countries c
    join iso on iso.code = c.iso2
  )
  select jsonb_build_object(
    'city', (select name from city),
    'country', (select iso2 from country),
    'country_name', (select name_en from country),
    'regions', coalesce((select jsonb_agg(jsonb_build_object('code', r.code, 'name_en', r.name_en) order by r.sort)
                         from oe_ref_regions r
                         where r.code = any(array(select unnest(c.region_codes) from country c))), '[]'::jsonb));
$function$;
REVOKE ALL ON FUNCTION public.oe_member_home(uuid) FROM PUBLIC, anon;
GRANT EXECUTE ON FUNCTION public.oe_member_home(uuid) TO authenticated, service_role;