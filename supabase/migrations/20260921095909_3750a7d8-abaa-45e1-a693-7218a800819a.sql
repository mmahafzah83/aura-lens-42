...
BEGIN
  ...
  ), ranked AS (SELECT v.*,row_number() OVER(PARTITION BY lane_final,purpose ORDER BY CASE WHEN lane_final='act' THEN deadline END ASC NULLS LAST,created_at DESC) purpose_rank,row_number() OVER(PARTITION BY lane_final ORDER BY CASE WHEN lane_final='act' THEN deadline END ASC NULLS LAST,created_at DESC) lane_rank FROM valid v),
  interleaved AS (SELECT r.*,((r.purpose_rank-0.5)/GREATEST(COALESCE((v_weights->>r.purpose)::numeric,0.01),0.01)) interleave_rank FROM ranked r), selected AS (SELECT i.* FROM interleaved i WHERE i.lane_final='act' OR i.lane_rank<=3)
