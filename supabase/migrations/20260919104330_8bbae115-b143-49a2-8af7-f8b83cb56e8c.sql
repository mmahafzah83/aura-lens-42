ALTER TABLE public.oe_matches
  ...
  ADD COLUMN IF NOT EXISTS eligibility_unknowns text[] NOT NULL DEFAULT '{}';

DO $$ BEGIN
  IF NOT EXISTS (SELECT 1 FROM pg_constraint WHERE conname='oe_matches_eligibility_outcome_check') THEN
    ALTER TABLE public.oe_matches ADD CONSTRAINT oe_matches_eligibility_outcome_check
      CHECK (eligibility_outcome IS NULL OR eligibility_outcome IN ('excluded','unknown','eligible'));
  END IF;
END $$;
...
BEGIN
  ...
  WITH latest AS (
    ...
    -- A stretch is a stretch even in his own sector: the build test runs first.
    SELECT f.*,
      CASE
        WHEN v_priority='bigger_seat' AND v_level_i IS NOT NULL AND f.level_i IS NOT NULL AND f.level_i > v_level_i THEN 'build'
