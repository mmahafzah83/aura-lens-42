...
BEGIN
  ...
  WITH latest AS (
    SELECT DISTINCT ON (m.opportunity_id) m.id,m.opportunity_id,m.scores,m.eligibility_outcome,COALESCE(m.eligibility_fail,'{}'::text[]) AS fails
    ...
    SELECT l.id,l.opportunity_id,l.eligibility_outcome,l.fails,o.chair_type,o.discovery_kind,o.sector,o.level_band,
    ...
    SELECT f.*,
      CASE
        WHEN (COALESCE(f.eligibility_outcome,'unknown') = 'excluded' OR array_length(f.fails,1) > 0) THEN
          CASE WHEN f.core_sector OR f.has_cited_evidence THEN 'strength' ELSE 'explore' END
        WHEN v_priority = 'bigger_seat'
             AND ((v_level_i IS NOT NULL AND f.level_i IS NOT NULL AND f.level_i > v_level_i) OR NOT f.core_sector)
             AND f.bridge
          THEN 'build'
        WHEN v_priority='known_for_one' AND f.chair_type IN ('speaking','media','learning') AND f.bridge THEN 'build'
        WHEN v_priority='new_rooms' AND (f.chair_type IN ('room','advisory') OR f.discovery_kind='corporate_event_inference') AND f.bridge THEN 'build'
        WHEN v_priority='out_of_sector' AND NOT f.core_sector AND f.bridge THEN 'build'
